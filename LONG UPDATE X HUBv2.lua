...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì LONG HYPER X đã trở lại!!!!!<Color=/>"):Display()
    wait(.14)
...
local Window = OrionLib:MakeWindow({Name = "LONG HYPER X", HidePremium = false, IntroText = "NightHub Library", SaveConfig = true, ConfigFolder = "LONG HYPER X"})
...
OrionLib:MakeNotification({
	Name = "LONG HYPER X",
	Content = "LONG HYPER X Not Skidder, Please don't says",
	Image = "rbxassetid://4483345998",
	Time = 25
})
...
W:AddLabel("[+] LONG HYPER X is back!")
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "LONG HYPER X(Noitification)",
